...
async function updateCoinGeckoTokensValue() {
  try {
    ...
    if (localStorage.getItem('btc') || isMoreThanWeek()) return

    if (!Array.isArray(tokens)) {
      throw new Error('Expected an array but received: ' + typeof tokens)
    }
    const tokenMap: Record<string, string> = tokens.reduce(
      (map: Record<string, string>, token: Token) => {
      ...
      if (tokenId) {
        ...
        if (matchingToken) {
          if (typeof matchingToken.current_price === 'number') {
            ...
            token.balanceValue = formatBalanceValue(
              matchingToken.current_price,
...
function formatBalanceValue(price: number, quantity: number): string {
  ...
  if (result > 10000) return result.toExponential(3)
  else return result.toPrecision(2)
}
